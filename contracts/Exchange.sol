...
contract Exchange is ERC20{
    ...
    constructor(address token) ERC20("ETH TOKEN LP TOKEN", "lpETHTOKEN"){
        require (token!=address(0),"Token address is null");
        tokenAddress=token;
    }
    ...
    function addLiquidity(uint256 amountOfToken) public payable returns (uint256) {
        ...
        require(amountOfToken >= minTokenAmountRequired, " Insufficient tokens provided");
        // Transfer the token from the user to the exchange
    ...
    function removeLiquidity(uint256 amountOfLPTokens) public returns(uint256,uint256){
        require(amountOfLPTokens>0,"Amount of tokens should be greater than 0");
        uint256 ethReserveBalance = address(this).balance;
    ...
    function getOutputAmountFromSwap(
        ...
        require (inputReserve > 0 && outputReserve > 0, "Reserve cannot be 0");
    ...
    function ethToTokenSwap(uint256 minTokensToReceive) public payable {
        ...
        require(tokensToReceive >= minTokensToReceive, "Insufficient tokens received than minimum");
